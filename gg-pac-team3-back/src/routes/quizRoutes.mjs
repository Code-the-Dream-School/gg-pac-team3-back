import express from 'express';
import { createQuiz, getQuiz, getAllQuizzesForLesson, updateQuiz, deleteQuiz } from '../controllers/quizController.mjs';

const router = express.Router();

// router.get('/:courseId/lessons/:lessonId', getLesson);
// Route to create a quiz for a specific lesson
router.post('/:courseId/lessons/:lessonId/quizzes', createQuiz);

// Route to get a specific quiz by ID
router.get('/:courseId/lessons/:lessonId/quizzes/:quizId', getQuiz);

// Route to get all quizzes for a specific lesson
router.get('/:courseId/lessons/:lessonId/quizzes', getAllQuizzesForLesson);

// Route to update a specific quiz by ID
router.patch('/:courseId/lessons/:lessonId/quizzes/:quizId', updateQuiz);

// Route to delete a specific quiz by ID
router.delete('/:courseId/lessons/:lessonId/quizzes/:quizId', deleteQuiz);

export default router;
